...
There exists many tools for managing this process and there also exists many software testing tools. However low amount of them is focused on _RESTful APIs_. I will focus on some of tools for description of testing _RESTful API_ in the next chapter.
...
If I consider _RESTful API_ as just another interface then it should be testable using both methods - _white box_ and _black box_. However what does it mean specificaly for _RESTful APIs_? One possibility is that it is usable in the same way. If the implementation is known then tests can be built using _white box_ to test edge cases.
